#!/usr/bin/env python3
"""Algorithme génétique avec plusieurs oiseaux simultanés dans la même partie"""
...
class MultiGeneticGame:
    """Jeu avec plusieurs oiseaux génétiques simultanés"""
    ...
    def __init__(self, population_weights: List[List[float]], seed: int = None):
        ...
        # Initialiser pygame
        pygame.init()
        pygame.display.set_caption("Algorithme Génétique Multi-Oiseaux")
    ...
    def run_generation(self):
        ...
        # La fitness de chaque oiseau est déjà calculée individuellement
        # Pas besoin de modifier ici, chaque oiseau garde sa propre fitness
        pass
...
def run_multi_visual_evolution():
    """Lance l'évolution avec visualisation multi-oiseaux"""
    print("🧬 ALGORITHME GÉNÉTIQUE MULTI-OISEAUX")
    print("=" * 60)
    print("Tous les oiseaux jouent simultanément!")
    ...
    while True:
        ...
        game = MultiGeneticGame(population_weights, seed=12345 + generation)
        ...
        for i, (weights, fitness, frames) in enumerate(results[:5]):
        ...
        response = input(f"\nContinuer à la génération {generation + 1}? (y/n): ")
        if response.lower() != "y":
            break
